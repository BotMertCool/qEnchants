    ...
    public void onButtonPress(InventoryClickEvent event) {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                player.setMetadata("CLICKED_BUTTON",new FixedMetadataValue(Enchants.getInstance(),player.getUniqueId().toString()));
                ...
                if (event.isCancelled()) {
                    Enchants.getInstance().getServer().getScheduler().runTaskLater(Enchants.getInstance(), player::updateInventory, 1L);
                }

                player.removeMetadata("CLICKED_BUTTON",Enchants.getInstance());
